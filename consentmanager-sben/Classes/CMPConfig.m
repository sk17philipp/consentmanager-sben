...
+ (void)setValues:(NSString *)domain addId:(NSString *)userId addAppName:(NSString *)appName addLanguage:(NSString*)language {
    ...
    consentToolAppName = [appName stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
...
+ (void)setConsentToolAppName:(NSString *)ctan{
    consentToolAppName = [ctan stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
}
...
+(NSString*)getConsentToolURLString:(NSString*)consent{
    if(consent && ![consent containsString:@"null"]){
        ...
        return [NSString stringWithFormat:ConsentToolURLUnformatted, consentToolDomain, consentToolId, consentToolAppName, @"", [CMPConfig idfa], consentToolLanguage];
    }
}
